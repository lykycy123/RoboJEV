...
控制台支持六个任务，包括新增的错位双门框，以及规则策略或 JEV、配对批量实验、种子、并发数和原始状态捕获。**正常使用仍默认原有输入。** 完整几何依赖现实中难以完整获取的仿真特权信息，仅作为可选对照。“配置 40 次对照实验”可对两种门框任务分别比较两种输入，详见[实验协议](docs/observation-experiment.md)和[已完成的配对结果、失败证据与七份真实录像](docs/observation-evaluation.md)。报告单独列出 API 中断，这轮小样本不能证明某种输入普遍更优。
...
空间输入对照包含单门框和错位双门框，每个任务比较默认原始输入与仅供仿真诊断的完整几何输入，各使用 10 个种子。四组成功数依次为 0/10、1/10、1/10、2/10；[页面对照区](https://lykycy123.github.io/RoboJEV/#observation)展示结果和七份原始试次录像。完整几何没有消除机械臂连杆碰撞，也不能凭这轮小样本宣称普遍更优。详见[配对分析](docs/observation-evaluation.md)。

原始对照中的 18 个 API 传输中断槽位通过代理桥重新提交（Slurm 作业 `647519`，重试 ID `4ab478783ebb4c7a9eead28c44495ce7`）。18/18 均在第一次决策的三次请求后中断，执行动作 0 次、物理失败 0 次、生成录像 0 份。另一次使用私有凭据的请求在收到 HTTP 响应前出现 `httpx.ReadError`；匿名请求可经代理到达端点，计算节点直连失败，因此尚不能判断是代理还是服务端关闭了认证请求。这些基础设施失败不计入已发布的 100 次评估。
